...
export default function LeavePolicyPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage("Leave policy saved and applied to all employees.");
    } catch (err) {
  ...
  return (
    <ProtectedRoute adminOnly>
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-black">Company Leave Policy</h1>
          <p className="mt-1 text-sm text-black">
            One policy for all employees. PL accrues monthly (end of month, based on joining date).
            CL/SL reset each financial year (April–March). PL carries forward.
          </p>
        </div>
        ...
        <Card>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="border-t border-slate-100 pt-6">
              ...
              <p className="mt-2 text-xs text-slate-500">
                CL is split across the financial year: half in Apr–Sep, half in Oct–Mar. Unused
                CL from Apr–Sep carries forward to Oct–Mar. SL resets each April.
              </p>
            </div>

            <Button type="submit" disabled={isSaving}>
              {isSaving ? "Saving..." : "Save Policy for All Employees"}
            </Button>
          </form>
        </Card>
      </div>
